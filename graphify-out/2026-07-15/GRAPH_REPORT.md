# Graph Report - Open_Cowork  (2026-07-15)

## Corpus Check
- 300 files · ~303,588 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4703 nodes · 11457 edges · 241 communities (191 shown, 50 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 88 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7fac2ae0`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Result|Result]]
- [[_COMMUNITY_lib.rs|lib.rs]]
- [[_COMMUNITY_crewStore.ts|crewStore.ts]]
- [[_COMMUNITY_registry.ts|registry.ts]]
- [[_COMMUNITY_String|String]]
- [[_COMMUNITY_crew_python_bridge.rs|crew_python_bridge.rs]]
- [[_COMMUNITY_CoworkView.tsx|CoworkView.tsx]]
- [[_COMMUNITY_AppHandle|AppHandle]]
- [[_COMMUNITY_App.tsx|App.tsx]]
- [[_COMMUNITY_ollamaClient.ts|ollamaClient.ts]]
- [[_COMMUNITY_Option|Option]]
- [[_COMMUNITY_tr|tr]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_Database|Database]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_ServiceError|ServiceError]]
- [[_COMMUNITY_ollama.rs|ollama.rs]]
- [[_COMMUNITY_workTaskCrewRuntime.ts|workTaskCrewRuntime.ts]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_artifact_pipeline.rs|artifact_pipeline.rs]]
- [[_COMMUNITY_useConfigStore|useConfigStore]]
- [[_COMMUNITY_safeInvoke|safeInvoke]]
- [[_COMMUNITY_main.py|main.py]]
- [[_COMMUNITY_Capability|Capability]]
- [[_COMMUNITY_String|String]]
- [[_COMMUNITY_coworkStore.ts|coworkStore.ts]]
- [[_COMMUNITY_configStore.ts|configStore.ts]]
- [[_COMMUNITY_context.rs|context.rs]]
- [[_COMMUNITY_cowork_features.rs|cowork_features.rs]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_chatStore.ts|chatStore.ts]]
- [[_COMMUNITY_mcp.rs|mcp.rs]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_openaiCompatibleClient.ts|openaiCompatibleClient.ts]]
- [[_COMMUNITY_engineStore.ts|engineStore.ts]]
- [[_COMMUNITY_Option|Option]]
- [[_COMMUNITY_personalityStore.ts|personalityStore.ts]]
- [[_COMMUNITY_terminal_sessions.rs|terminal_sessions.rs]]
- [[_COMMUNITY_chatAttachments.ts|chatAttachments.ts]]
- [[_COMMUNITY_CrewLiveMonitor.tsx|CrewLiveMonitor.tsx]]
- [[_COMMUNITY_EventEnvelope|EventEnvelope]]
- [[_COMMUNITY_office_integration.rs|office_integration.rs]]
- [[_COMMUNITY_LeftSidebar.tsx|LeftSidebar.tsx]]
- [[_COMMUNITY_toolOrchestrator.ts|toolOrchestrator.ts]]
- [[_COMMUNITY_Vec|Vec]]
- [[_COMMUNITY_claude_code_bridge.rs|claude_code_bridge.rs]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_AuditEvent|AuditEvent]]
- [[_COMMUNITY_file_safety.rs|file_safety.rs]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_Message|Message]]
- [[_COMMUNITY_audit_service.rs|audit_service.rs]]
- [[_COMMUNITY_projectStore.ts|projectStore.ts]]
- [[_COMMUNITY_db.rs|db.rs]]
- [[_COMMUNITY_validate-product-docs.mjs|validate-product-docs.mjs]]
- [[_COMMUNITY_insightsStore.ts|insightsStore.ts]]
- [[_COMMUNITY_crew_provider_health_check|crew_provider_health_check]]
- [[_COMMUNITY_tauri.conf.json|tauri.conf.json]]
- [[_COMMUNITY_terminalStore.ts|terminalStore.ts]]
- [[_COMMUNITY_properties|properties]]
- [[_COMMUNITY_registry.ts|registry.ts]]
- [[_COMMUNITY_RunPanel.tsx|RunPanel.tsx]]
- [[_COMMUNITY_QueryEngine|QueryEngine]]
- [[_COMMUNITY_.open_in_memory|.open_in_memory]]
- [[_COMMUNITY_validate-agent-discipline.mjs|validate-agent-discipline.mjs]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_i18n-audit.mjs|i18n-audit.mjs]]
- [[_COMMUNITY_messageDisplay.ts|messageDisplay.ts]]
- [[_COMMUNITY_globalSearch.ts|globalSearch.ts]]
- [[_COMMUNITY_RequestContext|RequestContext]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_Feature 2 Zusammenklapp-Buttons für Task-Chats|Feature 2: Zusammenklapp-Buttons für Task-Chats]]
- [[_COMMUNITY_lifecycle.ts|lifecycle.ts]]
- [[_COMMUNITY_ollamaStreaming.ts|ollamaStreaming.ts]]
- [[_COMMUNITY_UI-Anforderungsliste fuer Open_Cowork|UI-Anforderungsliste fuer Open_Cowork]]
- [[_COMMUNITY_memoryStore.ts|memoryStore.ts]]
- [[_COMMUNITY_workTasksStore.ts|workTasksStore.ts]]
- [[_COMMUNITY_terminal_backends.rs|terminal_backends.rs]]
- [[_COMMUNITY_INDEX|INDEX.md]]
- [[_COMMUNITY_5. Anforderungen pro Ansicht|5. Anforderungen pro Ansicht]]
- [[_COMMUNITY_check-budgets.mjs|check-budgets.mjs]]
- [[_COMMUNITY_logStore.ts|logStore.ts]]
- [[_COMMUNITY_memory_engine.rs|memory_engine.rs]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_SkillPanel.tsx|SkillPanel.tsx]]
- [[_COMMUNITY_useTerminalStore|useTerminalStore]]
- [[_COMMUNITY_chatProvider.ts|chatProvider.ts]]
- [[_COMMUNITY_taskStore.ts|taskStore.ts]]
- [[_COMMUNITY_asString|asString]]
- [[_COMMUNITY_next_run_from_expression|next_run_from_expression]]
- [[_COMMUNITY_LlmProfilesPanel.tsx|LlmProfilesPanel.tsx]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_Implementation Status ✅ COMPLETE|Implementation Status: ✅ COMPLETE]]
- [[_COMMUNITY_Changes Made|Changes Made]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_skill_engine.rs|skill_engine.rs]]
- [[_COMMUNITY_process_manager.rs|process_manager.rs]]
- [[_COMMUNITY_Fix for Per-ChatPer-Task Permission Mode and Allowed Directories|Fix for Per-Chat/Per-Task Permission Mode and Allowed Directories]]
- [[_COMMUNITY_5.3 Plan-, Approval- und Task-UI|5.3 Plan-, Approval- und Task-UI]]
- [[_COMMUNITY_Feature Implementierungsvergleich|Feature Implementierungsvergleich]]
- [[_COMMUNITY_Open Cowork|Open Cowork]]
- [[_COMMUNITY_insights.rs|insights.rs]]
- [[_COMMUNITY_Current Architecture|Current Architecture]]
- [[_COMMUNITY_FEHLENDE_FEATURES_IM_CODE|FEHLENDE_FEATURES_IM_CODE.md]]
- [[_COMMUNITY_duckduckgo-websearch-server.mjs|duckduckgo-websearch-server.mjs]]
- [[_COMMUNITY_documentWorkspaceStore.ts|documentWorkspaceStore.ts]]
- [[_COMMUNITY_gateway_health|gateway_health]]
- [[_COMMUNITY_worker_sandbox.rs|worker_sandbox.rs]]
- [[_COMMUNITY_modelCapabilities.ts|modelCapabilities.ts]]
- [[_COMMUNITY_Development und Betrieb|Development und Betrieb]]
- [[_COMMUNITY_TerminalDock.tsx|TerminalDock.tsx]]
- [[_COMMUNITY_README|README.md]]
- [[_COMMUNITY_Decisions|Decisions]]
- [[_COMMUNITY_workTasksStore.ts|workTasksStore.ts]]
- [[_COMMUNITY_build.rs|build.rs]]
- [[_COMMUNITY_Architektur|Architektur]]
- [[_COMMUNITY_Decisions|Decisions]]
- [[_COMMUNITY_Ollama Konfiguration|Ollama Konfiguration]]
- [[_COMMUNITY_generateIndex|generateIndex]]
- [[_COMMUNITY_Desktop Smoke Test|Desktop Smoke Test]]
- [[_COMMUNITY_Decisions|Decisions]]
- [[_COMMUNITY_buildProductDoc|buildProductDoc]]
- [[_COMMUNITY_parseBlock|parseBlock]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_Open Cowork App|Open Cowork App]]
- [[_COMMUNITY_doctor.mjs|doctor.mjs]]
- [[_COMMUNITY_WorkTaskRow|WorkTaskRow]]
- [[_COMMUNITY_SettingsView.tsx|SettingsView.tsx]]
- [[_COMMUNITY_Runtime Compatibility|Runtime Compatibility]]
- [[_COMMUNITY_5.2 Cowork-Hauptansicht|5.2 Cowork-Hauptansicht]]
- [[_COMMUNITY_Contributing|Contributing]]
- [[_COMMUNITY_Product Documentation Index|Product Documentation Index]]
- [[_COMMUNITY_useTerminalStore|useTerminalStore]]
- [[_COMMUNITY_work_task_upsert|work_task_upsert]]
- [[_COMMUNITY_Decisions|Decisions]]
- [[_COMMUNITY_6. Querschnittliche UI-Anforderungen|6. Querschnittliche UI-Anforderungen]]
- [[_COMMUNITY_desktop-smoke.mjs|desktop-smoke.mjs]]
- [[_COMMUNITY_default.json|default.json]]
- [[_COMMUNITY_pipelineStore.ts|pipelineStore.ts]]
- [[_COMMUNITY_Claude-Code Feature Adoption (Open_Cowork)|Claude-Code Feature Adoption (Open_Cowork)]]
- [[_COMMUNITY_3. Gestaltungsprinzipien|3. Gestaltungsprinzipien]]
- [[_COMMUNITY_uniqueStrings|uniqueStrings]]
- [[_COMMUNITY_memoryStore.ts|memoryStore.ts]]
- [[_COMMUNITY_Current Routes|Current Routes]]
- [[_COMMUNITY_4. Informationsarchitektur|4. Informationsarchitektur]]
- [[_COMMUNITY_Copilot Project Instructions|Copilot Project Instructions]]
- [[_COMMUNITY_Memory And Skill Discipline|Memory And Skill Discipline]]
- [[_COMMUNITY_Skills Index|Skills Index]]
- [[_COMMUNITY_openaiCompatibleClient.test.ts|openaiCompatibleClient.test.ts]]
- [[_COMMUNITY_skill-and-memory.instructions|skill-and-memory.instructions.md]]
- [[_COMMUNITY_Agent Memory Test Tasks|Agent Memory Test Tasks]]
- [[_COMMUNITY_ollamaClient.test.ts|ollamaClient.test.ts]]
- [[_COMMUNITY_McpView.tsx|McpView.tsx]]
- [[_COMMUNITY_McpView.tsx|McpView.tsx]]
- [[_COMMUNITY_ollamaStreaming.test.ts|ollamaStreaming.test.ts]]
- [[_COMMUNITY_tsconfig.json|tsconfig.json]]
- [[_COMMUNITY_refactor-settings.js|refactor-settings.js]]
- [[_COMMUNITY_refactor-ui.js|refactor-ui.js]]
- [[_COMMUNITY_validateProductDoc|validateProductDoc]]
- [[_COMMUNITY_AGENTS|AGENTS.md]]
- [[_COMMUNITY_registry.filesystem.test.ts|registry.filesystem.test.ts]]
- [[_COMMUNITY_documentWorkspaceStore.test.ts|documentWorkspaceStore.test.ts]]
- [[_COMMUNITY_agent-memory-notes-example|agent-memory-notes-example.md]]
- [[_COMMUNITY_queryEngine.approval.test.ts|queryEngine.approval.test.ts]]
- [[_COMMUNITY_file-access-policy|file-access-policy.md]]
- [[_COMMUNITY_followUpPrompt.ts|followUpPrompt.ts]]
- [[_COMMUNITY_createDesktopscreenshotAttachment|createDesktopscreenshotAttachment]]
- [[_COMMUNITY_ui-quality.spec.ts|ui-quality.spec.ts]]
- [[_COMMUNITY_verify.mjs|verify.mjs]]
- [[_COMMUNITY_sensitive_data.rs|sensitive_data.rs]]
- [[_COMMUNITY_LlmProfilesPanel.tsx|LlmProfilesPanel.tsx]]
- [[_COMMUNITY_ChatTurnRequest|ChatTurnRequest]]
- [[_COMMUNITY_Audit Integrity and Threat Model Contract|Audit Integrity and Threat Model Contract]]
- [[_COMMUNITY_GuidedOnboarding.tsx|GuidedOnboarding.tsx]]
- [[_COMMUNITY_resolveShellNavigationTarget|resolveShellNavigationTarget]]
- [[_COMMUNITY_normalizeDesktopCoordinateSpace|normalizeDesktopCoordinateSpace]]
- [[_COMMUNITY_Release Supply Chain Contract|Release Supply Chain Contract]]
- [[_COMMUNITY_README|README.md]]
- [[_COMMUNITY_5.2 Cowork-Hauptansicht|5.2 Cowork-Hauptansicht]]
- [[_COMMUNITY_5.3 Plan-, Approval- und Task-UI|5.3 Plan-, Approval- und Task-UI]]
- [[_COMMUNITY_8. Accessibility|8. Accessibility]]
- [[_COMMUNITY_7. Desktop- und Fensterverhalten|7. Desktop- und Fensterverhalten]]
- [[_COMMUNITY_9. Visuelle und textliche Anforderungen|9. Visuelle und textliche Anforderungen]]
- [[_COMMUNITY_normalizeDesktopCoordinateSpace|normalizeDesktopCoordinateSpace]]
- [[_COMMUNITY_CrewRoleBindingRow|CrewRoleBindingRow]]
- [[_COMMUNITY_CrewRunEventRow|CrewRunEventRow]]
- [[_COMMUNITY_EngineRunArtifactRow|EngineRunArtifactRow]]
- [[_COMMUNITY_EngineRunCheckpointRow|EngineRunCheckpointRow]]
- [[_COMMUNITY_EngineRunEventRow|EngineRunEventRow]]
- [[_COMMUNITY_EngineRunRow|EngineRunRow]]
- [[_COMMUNITY_McpCallResponse|McpCallResponse]]
- [[_COMMUNITY_McpProbeResponse|McpProbeResponse]]
- [[_COMMUNITY_McpRuntimeServerStatus|McpRuntimeServerStatus]]
- [[_COMMUNITY_OllamaConfig|OllamaConfig]]
- [[_COMMUNITY_ProcessStartResult|ProcessStartResult]]
- [[_COMMUNITY_ProcessStatusResult|ProcessStatusResult]]
- [[_COMMUNITY_RuntimeInstructionRow|RuntimeInstructionRow]]
- [[_COMMUNITY_SupportBundleResponse|SupportBundleResponse]]
- [[_COMMUNITY_WorkerSandboxRow|WorkerSandboxRow]]
- [[_COMMUNITY_workTaskCrewRuntime.ts|workTaskCrewRuntime.ts]]
- [[_COMMUNITY_workTasksStore.ts|workTasksStore.ts]]
- [[_COMMUNITY_pipelineStore.ts|pipelineStore.ts]]
- [[_COMMUNITY_crewHandler.ts|crewHandler.ts]]
- [[_COMMUNITY_chatStore.ts|chatStore.ts]]
- [[_COMMUNITY_chatProvider.ts|chatProvider.ts]]
- [[_COMMUNITY_QueryEngine|QueryEngine]]
- [[_COMMUNITY_ProjectView.tsx|ProjectView.tsx]]
- [[_COMMUNITY_taskStore.ts|taskStore.ts]]
- [[_COMMUNITY_SkillPanel.tsx|SkillPanel.tsx]]
- [[_COMMUNITY_messageDisplay.ts|messageDisplay.ts]]
- [[_COMMUNITY_webSearchSources.ts|webSearchSources.ts]]
- [[_COMMUNITY_claudeBridge.ts|claudeBridge.ts]]
- [[_COMMUNITY_InsightsPanel.tsx|InsightsPanel.tsx]]
- [[_COMMUNITY_followUpPrompt.ts|followUpPrompt.ts]]
- [[_COMMUNITY_processStore.ts|processStore.ts]]
- [[_COMMUNITY_AnthropicAPIError|AnthropicAPIError]]
- [[_COMMUNITY_GuidedOnboarding.tsx|GuidedOnboarding.tsx]]
- [[_COMMUNITY_taskTemplatesStore.ts|taskTemplatesStore.ts]]
- [[_COMMUNITY_Current Architecture|Current Architecture]]
- [[_COMMUNITY_CrewLiveMonitor.test.tsx|CrewLiveMonitor.test.tsx]]
- [[_COMMUNITY_README|README.md]]
- [[_COMMUNITY_schedulerUtils.ts|schedulerUtils.ts]]
- [[_COMMUNITY_5.2 Cowork-Hauptansicht|5.2 Cowork-Hauptansicht]]
- [[_COMMUNITY_5.3 Plan-, Approval- und Task-UI|5.3 Plan-, Approval- und Task-UI]]
- [[_COMMUNITY_8. Accessibility|8. Accessibility]]
- [[_COMMUNITY_7. Desktop- und Fensterverhalten|7. Desktop- und Fensterverhalten]]
- [[_COMMUNITY_9. Visuelle und textliche Anforderungen|9. Visuelle und textliche Anforderungen]]
- [[_COMMUNITY_notifications.ts|notifications.ts]]

## God Nodes (most connected - your core abstractions)
1. `Database` - 365 edges
2. `tr()` - 105 edges
3. `useConfigStore` - 46 edges
4. `QueryEngine` - 34 edges
5. `ServiceError` - 33 edges
6. `CoworkView()` - 29 edges
7. `scripts` - 27 edges
8. `RequestContext` - 26 edges
9. `collect_crew_governance_payload()` - 25 edges
10. `CrewExecuteRequest` - 24 edges

## Surprising Connections (you probably didn't know these)
- `LlmProfilesPanel()` --calls--> `hasTauriRuntime()`  [INFERRED]
  app/src/components/LlmProfilesPanel.tsx → app/src/stores/coworkStore.ts
- `SupportBundlePanel()` --calls--> `tr()`  [EXTRACTED]
  app/src/components/SettingsView.tsx → app/src/i18n.ts
- `TasksView()` --calls--> `resolveCrewRuntimeConfig()`  [EXTRACTED]
  app/src/components/TasksView.tsx → app/src/stores/crewStore.ts
- `TasksView()` --calls--> `resolveExternalProviderConfig()`  [EXTRACTED]
  app/src/components/TasksView.tsx → app/src/stores/crewStore.ts
- `CrewRuntimePanel()` --calls--> `hasTauriRuntime()`  [INFERRED]
  app/src/components/crew/CrewRuntimePanel.tsx → app/src/stores/coworkStore.ts

## Import Cycles
- None detected.

## Communities (241 total, 50 thin omitted)

### Community 0 - "Result"
Cohesion: 0.05
Nodes (130): ArtifactExportRow, backend_delete(), backend_ensure_local(), backend_exec(), backend_list(), backend_upsert(), build_provider_chat_urls(), build_provider_model_urls() (+122 more)

### Community 1 - "lib.rs"
Cohesion: 0.20
Nodes (14): apply_provider_headers(), connector_test_reachability(), ConnectorReachabilityRequest, ConnectorReachabilityResponse, interpret_connector_status(), probe_connector_method(), provider_get_response_text(), provider_post_chat_probe() (+6 more)

### Community 2 - "crewStore.ts"
Cohesion: 0.04
Nodes (76): CrewControlPlanePanel(), formatTimestamp(), Props, CrewGovernancePanel(), formatApprovalStatus(), formatApprovalType(), formatTimestamp(), GOVERNANCE_MODES (+68 more)

### Community 3 - "registry.ts"
Cohesion: 0.03
Nodes (62): agentTool, askUserTool, AskUserToolInput, bashTool, copyPathTool, createDirectoryTool, deleteFileTool, DesktopActionResponse (+54 more)

### Community 4 - "String"
Cohesion: 0.06
Nodes (64): chat_turn(), chat_turn_stream(), ChatTurnRequest, crew_approval_create(), crew_role_binding_upsert(), CrewApprovalCreateRequest, CrewMemoryEntryPayload, CrewMemoryPayload (+56 more)

### Community 5 - "crew_python_bridge.rs"
Cohesion: 0.13
Nodes (58): build_status_from_json(), command_available(), crew_runtime_bootstrap(), crew_runtime_execute_request(), crew_runtime_status(), crew_runtime_status_internal(), crew_runtime_validate_definition(), CrewPythonBridge (+50 more)

### Community 6 - "CoworkView.tsx"
Cohesion: 0.08
Nodes (45): AskUserOption, AskUserPromptModel, buildChatExportPayload(), buildEngineUserInput(), buildProjectInstructionsPromptContext(), buildProjectLinkPromptContext(), ChatExportFormat, clipVerboseText() (+37 more)

### Community 7 - "AppHandle"
Cohesion: 0.05
Nodes (65): ArtifactVersionRow, build_exec_command_text(), crew_runs_list(), CrewProviderHealthCheckRequest, CrewProviderHealthCheckResponse, CrewProviderModelsRequest, CrewRunHistoryRow, db_save_thread() (+57 more)

### Community 8 - "App.tsx"
Cohesion: 0.13
Nodes (20): queryClient, isSensitiveKey(), normalizeKey(), redactAtDepth(), redactRecord(), redactSensitiveData(), redactText(), redactCrewExecutionLog() (+12 more)

### Community 9 - "ollamaClient.ts"
Cohesion: 0.05
Nodes (65): blockContentToText(), buildOllamaChatRequest(), buildStartTagRegex(), buildToolLookup(), canDelayVisibleStream(), canonicalizeArgumentKey(), canUseTauriInvoke(), clipOllamaDebugText() (+57 more)

### Community 10 - "Option"
Cohesion: 0.09
Nodes (15): streamOllamaMessagesMock, streamOllamaMessagesMock, chunk(), DEFAULT_ORCHESTRATOR_CONFIG, ToolExecutionEvent, ToolExecutionResult, ToolOrchestrator, ToolOrchestratorConfig (+7 more)

### Community 11 - "tr"
Cohesion: 0.09
Nodes (35): configure_pdfium_search_paths(), document_render_preview(), enforce_file_tool_policy(), ensure_run_file_access(), execute_task(), export_save_text_file(), fs_copy_path(), fs_create_directory() (+27 more)

### Community 12 - "index.ts"
Cohesion: 0.12
Nodes (28): OllamaEngineConfig, sampleOllamaMessage(), applyToolResultBudget(), autoCompact(), createTokenBudget(), estimateConversationTokens(), estimateTokens(), fallbackCompact() (+20 more)

### Community 13 - "Database"
Cohesion: 0.07
Nodes (39): App(), AppRoutes(), BackendPolicyState, confirmAppClose(), CoworkView, CrewView, FeaturesView, hasRunningWork() (+31 more)

### Community 14 - "index.ts"
Cohesion: 0.10
Nodes (46): PermissionDecision, accumulateUsage(), AgentToolProgress, AssistantMessage, AttachmentMessage, BashProgress, ContentBlockDelta, ContentBlockImage (+38 more)

### Community 15 - "ServiceError"
Cohesion: 0.08
Nodes (25): api_error_response_matches_contract_shape(), ApiError, ApiErrorResponse, conflict_error_maps_to_http_conflict(), current_tauri_result_conversion_keeps_existing_string_boundary(), forbidden_error_maps_to_permission_denied(), internal_error_does_not_leak_source_to_safe_message_or_tauri_conversion(), into_tauri_result() (+17 more)

### Community 16 - "ollama.rs"
Cohesion: 0.13
Nodes (52): ollama_health_check(), build_chat_messages(), build_chat_prompt(), build_chat_turn_response(), build_chat_turn_response_preserves_tool_calls(), build_http_client(), chat_turn(), chat_turn_stream() (+44 more)

### Community 17 - "workTaskCrewRuntime.ts"
Cohesion: 0.18
Nodes (8): engine_runs_capture_gateway_metadata(), EngineRunArtifactRow, EngineRunRow, InsightsEventRow, map_engine_run_artifact_row(), map_engine_run_row(), map_insights_row(), Row

### Community 18 - "properties"
Cohesion: 0.05
Nodes (48): items, type, items, type, items, type, pattern, type (+40 more)

### Community 19 - "artifact_pipeline.rs"
Cohesion: 0.16
Nodes (45): ArtifactParseResponse, bind_pdfium(), extract_pdf_text_with_pdfium(), extract_text_for_llm(), extract_text_for_llm_limited(), parse_artifact(), parse_binary(), parse_csv() (+37 more)

### Community 20 - "useConfigStore"
Cohesion: 0.05
Nodes (18): CrewApprovalRow, CrewDefinitionRow, CrewRoleBindingRow, CrewRunEventRow, Database, map_worker_sandbox_row(), memory_search_ranks_natural_language_terms_instead_of_requiring_exact_phrase(), MemoryEntryRow (+10 more)

### Community 21 - "safeInvoke"
Cohesion: 0.13
Nodes (31): append_with_limit(), fetch_public_text(), is_allowed_text_content_type(), is_followable_redirect(), is_public_ipv4(), is_public_ipv6(), normalize_content_type(), origin_for_audit() (+23 more)

### Community 22 - "main.py"
Cohesion: 0.10
Nodes (41): agent_display_name(), build_agent(), build_governance_note(), build_llm(), build_memory_note(), build_task_description(), classify_runtime_error(), clean_log_text() (+33 more)

### Community 23 - "Capability"
Cohesion: 0.11
Nodes (24): Capability, capability_deserializes_from_policy_name(), capability_response_serializes_policy_names(), CapabilityCategory, CapabilityDescriptor, CapabilityResponse, CapabilityStatus, dangerous_capabilities_are_disabled_by_default_when_supported() (+16 more)

### Community 24 - "String"
Cohesion: 0.07
Nodes (14): CrewDefinitionVersionRow, EngineRunCheckpointRow, LearningOutcomeRow, ManagedProcessRow, MemoryProviderRow, PersonalityRow, ProjectRow, RpcPipelineRow (+6 more)

### Community 25 - "coworkStore.ts"
Cohesion: 0.06
Nodes (36): BackendConnectorTestResponse, BackendScheduledRunRow, BackendScheduledTaskRow, buildPolicySyncRequest(), CLAUDE_TOOL_CAPABILITIES, ClaudeToolCapability, ClaudeToolPreset, ConnectorConfig (+28 more)

### Community 26 - "configStore.ts"
Cohesion: 0.15
Nodes (7): autoSaveSessionMock, buildSystemPromptWithMemoryMock, FakeQueryEngine, invokeMock, loadSessionMock, queryBarriers, queryCalls

### Community 27 - "context.rs"
Cohesion: 0.09
Nodes (19): Actor, ActorId, ActorKind, ActorRole, anonymous_fixture_has_no_actor_id(), ClientPlatform, context_serializes_with_camel_case_fields(), local_default_context_needs_no_network_config() (+11 more)

### Community 28 - "cowork_features.rs"
Cohesion: 0.17
Nodes (42): analyze_single_path(), apply_office_template_transform(), ArtifactVersionExportInput, build_artifact_field_rows(), build_workflow_headers(), build_workflow_rows(), build_workflow_totals(), compute_numeric_totals() (+34 more)

### Community 29 - "properties"
Cohesion: 0.05
Nodes (42): type, type, type, type, type, type, pattern, type (+34 more)

### Community 30 - "chatStore.ts"
Cohesion: 0.07
Nodes (47): ANTHROPIC_MODELS, AnthropicConfig, APIContentBlock, APIMessage, APIToolDef, calculateCost(), COST_PER_MILLION, sampleMessage() (+39 more)

### Community 31 - "mcp.rs"
Cohesion: 0.14
Nodes (48): local_docs_mcp_probe(), local_screenshot_mcp_probe(), mcp_probe(), mcp_runtime_restart(), mcp_runtime_start(), call_tool(), format_call_result(), McpCallRequest (+40 more)

### Community 32 - "properties"
Cohesion: 0.05
Nodes (41): type, items, type, type, pattern, type, items, type (+33 more)

### Community 33 - "openaiCompatibleClient.ts"
Cohesion: 0.08
Nodes (40): APIMessage, APIToolDef, blocksToUserContent(), buildEndpoint(), buildModelsEndpoint(), createAbortSignal(), extractReasoningContent(), extractReasoningDetailText() (+32 more)

### Community 34 - "engineStore.ts"
Cohesion: 0.06
Nodes (15): invokeMock, checkOllamaStatusMock, fetchOllamaModelsMock, invokeMock, saveDialogMock, appendRunEvent(), ChatHistorySeedMessage, EngineProvider (+7 more)

### Community 35 - "Option"
Cohesion: 0.05
Nodes (42): 10. Release-Gates, 11. Steuerung und Priorisierung, 12. Unmittelbar naechste Umsetzungsschritte, 1. Zielbild, 2. Verifizierter Ausgangspunkt und Fortschritt, 3. Produkt- und Architekturentscheidungen, 4. Ziel-Domaenen und neue Vertraege, 5. Themengebiete und Feature-Plan (+34 more)

### Community 36 - "personalityStore.ts"
Cohesion: 0.07
Nodes (30): CATEGORY_LABELS, CATEGORY_ORDER, CommandPalette(), FeaturesView(), isWorkbenchTab(), TABS, WorkbenchTab, formatDateTime() (+22 more)

### Community 37 - "terminal_sessions.rs"
Cohesion: 0.13
Nodes (37): close_terminal_session(), configure_shell_command(), create_terminal_session(), default_shell(), interrupt_terminal_session(), kill_terminal_session(), ManagedTerminalSession, pty_size() (+29 more)

### Community 38 - "chatAttachments.ts"
Cohesion: 0.21
Nodes (22): a_new_service_instance_continues_the_persisted_chain(), append_with_key(), AuditIntegrityEnvelope, calculate_mac(), canonical_record(), create_key_if_allowed(), encode_hex(), legacy_jsonl_service_preserves_existing_event_shape() (+14 more)

### Community 39 - "CrewLiveMonitor.tsx"
Cohesion: 0.13
Nodes (28): AgentStream, buildAgentStreams(), buildEntryMeta(), buildRollingWindowLines(), CATEGORY_LABELS, createEmptyCounts(), CrewLiveDisplayCategory, CrewLiveFilter (+20 more)

### Community 40 - "EventEnvelope"
Cohesion: 0.14
Nodes (17): event_envelope_serializes_with_contract_fields(), EventEnvelope, EventId, EventReplayMetadata, EventSequence, legacy_tauri_event_keeps_current_event_name(), noop_event_sink_accepts_envelopes(), DateTime (+9 more)

### Community 41 - "office_integration.rs"
Cohesion: 0.18
Nodes (32): detect_app(), detect_app_for_kind(), detect_office_apps(), document_format(), DocumentPreviewRequest, DocumentPreviewResponse, export_office_to_pdf(), find_in_path() (+24 more)

### Community 42 - "LeftSidebar.tsx"
Cohesion: 0.08
Nodes (38): active_toolset_inference_detects_custom_edits(), backend_file_mutations_respect_the_active_toolset_policy(), build_policy_tool_states(), build_toolset_policy(), canonical_policy_tool_id(), crew_tool_allowed_by_flags(), default_policy_enabled_tool_ids_vec(), default_policy_flags() (+30 more)

### Community 43 - "toolOrchestrator.ts"
Cohesion: 0.28
Nodes (19): append_test_event(), AuditVerificationState, decode_mac(), deletion_inside_the_retained_chain_is_detected(), legacy_prefix_can_be_migrated_but_remains_degraded(), legacy_record_after_signed_history_is_detected(), malformed_complete_record_is_detected(), modified_details_are_detected_and_block_subsequent_appends() (+11 more)

### Community 44 - "Vec"
Cohesion: 0.14
Nodes (25): normalizeSessions(), normalizeSessionSummary(), SessionLike, SessionSearchPanel(), sessionRecord, sessionSummary, toNumber(), SessionRecord (+17 more)

### Community 45 - "claude_code_bridge.rs"
Cohesion: 0.19
Nodes (23): ClaudeCodeBridge, ClaudeCodeCommandInfo, ClaudeCodeConfig, ClaudeCodeProcess, ClaudeCodeResponse, ClaudeCodeStatus, ClaudeCodeStreamChunk, ClaudeCodeToolInfo (+15 more)

### Community 46 - "properties"
Cohesion: 0.07
Nodes (31): pattern, type, type, items, type, items, type, properties (+23 more)

### Community 47 - "properties"
Cohesion: 0.07
Nodes (31): items, type, items, type, pattern, type, pattern, type (+23 more)

### Community 48 - "AuditEvent"
Cohesion: 0.17
Nodes (19): audit_event_serializes_with_snake_case_contract_fields(), AuditEvent, AuditId, AuditOutcome, AuditRiskClass, AuditTarget, denial_event_construction_preserves_high_risk_result(), noop_audit_sink_accepts_events() (+11 more)

### Community 49 - "file_safety.rs"
Cohesion: 0.16
Nodes (38): allowed(), backup_root(), BackupEntry, canonicalize_for_policy(), copy_directory_recursive(), copy_path(), create_backup_path(), create_directory() (+30 more)

### Community 50 - "properties"
Cohesion: 0.07
Nodes (30): items, type, type, items, type, pattern, type, type (+22 more)

### Community 51 - "Message"
Cohesion: 0.21
Nodes (13): audit_service_records_denial_event_without_tauri_runtime(), audit_service_records_success_event_without_tauri_runtime(), AuditService<S>, RecordingAuditSink, Into, Mutex, Vec, AuditEvent (+5 more)

### Community 52 - "audit_service.rs"
Cohesion: 0.07
Nodes (50): CrewRuntimePanel(), formatTimestamp(), hasTauriRuntimeMock, MessageStreamPanel(), MessageStreamPanelProps, MessageThinking(), MessageThinkingProps, MessageVerbose() (+42 more)

### Community 53 - "projectStore.ts"
Cohesion: 0.10
Nodes (26): AddProjectResourceInput, addUniqueResources(), DbProject, DbProjectResource, DeleteProjectOptions, generateId(), getEnabledProjectAttachments(), getEnabledProjectLinks() (+18 more)

### Community 54 - "db.rs"
Cohesion: 0.31
Nodes (14): aggregate_gateway_status(), build_local_gateway_subsystems(), check_audit_writable(), gateway_health(), gateway_payload(), gateway_probe(), gateway_provider_probe(), gateway_status() (+6 more)

### Community 55 - "validate-product-docs.mjs"
Cohesion: 0.08
Nodes (23): appRoutes, appRoutesFile, catalog, CATALOG_KIND_CONFIG, catalogDir, catalogIndexPath, catalogMarkdown, docs (+15 more)

### Community 56 - "insightsStore.ts"
Cohesion: 0.20
Nodes (18): addLocalEvent(), asArray(), asNullableString(), asNumber(), asRecord(), asString(), buildLocalSummary(), CategoryCount (+10 more)

### Community 57 - "crew_provider_health_check"
Cohesion: 0.07
Nodes (49): build_crew_memory_query(), build_effective_crew_tool_ids(), collect_crew_governance_payload(), collect_crew_memory_payload(), crew_agent_can_delegate(), crew_approval_resolve(), crew_execute(), crew_role_allows_execution() (+41 more)

### Community 58 - "tauri.conf.json"
Cohesion: 0.07
Nodes (26): app, security, trayIcon, windows, build, beforeBuildCommand, beforeDevCommand, devUrl (+18 more)

### Community 59 - "terminalStore.ts"
Cohesion: 0.06
Nodes (33): getSessionLabel(), getStatusLabel(), TerminalDock(), TerminalDockProps, xtermInstances, createSession(), invokeMock, runAiCommandMock (+25 more)

### Community 60 - "properties"
Cohesion: 0.08
Nodes (25): type, type, type, pattern, type, pattern, type, properties (+17 more)

### Community 61 - "registry.ts"
Cohesion: 0.09
Nodes (24): agentsCommand, clearCommand, commandRegistry, compactCommand, costCommand, cwdCommand, debugCommand, executeCommand() (+16 more)

### Community 62 - "RunPanel.tsx"
Cohesion: 0.10
Nodes (38): canOpenArtifact(), CoworkContextRail(), CoworkContextRailProps, STATUS_LABELS, baseProps, safeInvokeMock, task, toolStatusIcon() (+30 more)

### Community 63 - "QueryEngine"
Cohesion: 0.32
Nodes (11): archive_path(), audit_file_path(), audit_rotation_keeps_only_the_configured_archives(), has_audit_history(), retained_audit_paths(), rotate_if_needed(), Path, PathBuf (+3 more)

### Community 64 - ".open_in_memory"
Cohesion: 0.15
Nodes (6): delete_thread_cascades(), messages_round_trip(), migration_creates_tables(), project_thread_assignment_is_exclusive_and_delete_can_remove_threads(), ProjectResourceRow, projects_resources_and_threads_round_trip()

### Community 65 - "validate-agent-discipline.mjs"
Cohesion: 0.08
Nodes (19): args, checkNotes, descLower, expandedPrompts, frontmatter, memoryPathArgIndex, missing, missingKeywords (+11 more)

### Community 66 - "devDependencies"
Cohesion: 0.08
Nodes (25): devDependencies, autoprefixer, @axe-core/playwright, eslint, @eslint/js, eslint-plugin-react-hooks, eslint-plugin-react-refresh, globals (+17 more)

### Community 67 - "i18n-audit.mjs"
Cohesion: 0.09
Nodes (18): collectTsFindings(), deKeys, enKeys, files, germanInEnglishResources, ignoredDirs, isTestFile(), missingInDe (+10 more)

### Community 68 - "messageDisplay.ts"
Cohesion: 0.17
Nodes (26): AuditIntegrityReport, AuditIntegrityStatus, audit_integrity(), audit_tail(), bundle_is_whitelisted_manifested_and_contains_no_input_secrets(), bundle_rejects_relative_and_non_zip_targets(), BundleEntry, create() (+18 more)

### Community 69 - "globalSearch.ts"
Cohesion: 0.06
Nodes (42): ExternalProviderHealthCheckResult, ExternalProviderModelsResult, LlmProfilesPanel(), modelSuffix(), parseNumericInput(), ProfileHealthState, ProfileModelsState, PROVIDER_LABELS (+34 more)

### Community 70 - "RequestContext"
Cohesion: 0.13
Nodes (25): AuditSink, NoopAuditSink, Send, Sync, RequestContext, DateTime, Utc, EventSink (+17 more)

### Community 71 - "compilerOptions"
Cohesion: 0.09
Nodes (22): compilerOptions, allowImportingTsExtensions, baseUrl, erasableSyntaxOnly, ignoreDeprecations, jsx, lib, module (+14 more)

### Community 72 - "Feature 2: Zusammenklapp-Buttons für Task-Chats"
Cohesion: 0.09
Nodes (22): 1. Anforderungsliste (Requirements List), 2. Einzelne Tasks (Individual Tasks), 3. Qualitätskriterien (Quality Criteria), 4. Technische Hinweise, 5. Umsetzungsreihenfolge (Implementation Order), Anforderungen: Task-Chat Collapse & Projekt-Lösch-Button Entfernung, Betroffene Dateien, Chat-Typ „Task“ identifizieren (+14 more)

### Community 73 - "lifecycle.ts"
Cohesion: 0.09
Nodes (22): ApprovalResolution, McpServerInstatce, McpServerStatus, McpTratsportKind, PluginMatifest, providerAdapter, providerCapability, providerHealth (+14 more)

### Community 74 - "ollamaStreaming.ts"
Cohesion: 0.16
Nodes (21): buildChatPrompt(), buildResponse(), callOllamaGenerate(), canUseTauriInvoke(), ChatTurnRequest, ChatTurnResponse, createStreamId(), detectRiskyAction() (+13 more)

### Community 75 - "UI-Anforderungsliste fuer Open_Cowork"
Cohesion: 0.22
Nodes (8): 10. Abnahmekriterien fuer die UI, 11. Priorisierung fuer Open_Cowork, 12. Bezug zu bestehender Dokumentation, 1. Zweck und Scope, 2. UI-Ziele, Muss fuer den naechsten UI-Ausbau, Soll nachziehen, UI-Anforderungsliste fuer Open_Cowork

### Community 76 - "memoryStore.ts"
Cohesion: 0.20
Nodes (12): terminal_close(), terminal_create(), terminal_interrupt(), terminal_kill(), terminal_resize(), terminal_write(), TerminalCreateRequest, TerminalCreateResponse (+4 more)

### Community 77 - "workTasksStore.ts"
Cohesion: 0.07
Nodes (37): CrewExecutionLogRow, CrewExecutionResponse, CrewHistoryPanel(), CrewRunEventRow, CrewRunHistoryRow, formatTimestamp(), Props, randomId() (+29 more)

### Community 78 - "terminal_backends.rs"
Cohesion: 0.05
Nodes (87): attach_process_tree(), configure_process_tree(), ProcessTreeGuard, Child, Command, Option, Result, terminate_platform_tree() (+79 more)

### Community 79 - "INDEX.md"
Cohesion: 0.06
Nodes (16): Crew Task Import Control, Task Create Panel, Task Detail Pane, Task List Pane, Task Run Toolbar, Task Scheduler Panel, Crew Task Import Flow, WorkTask Run And Schedule Flow (+8 more)

### Community 80 - "5. Anforderungen pro Ansicht"
Cohesion: 0.18
Nodes (11): 5.1 Welcome Screen, 5.4 Features-Ansicht, 5.5 Settings-Ansicht, 5. Anforderungen pro Ansicht, UI-020 Einstieg und Orientierung, UI-021 Guided Onboarding, UI-050 Feature-Katalog, UI-051 Aktivierung und Konfiguration (+3 more)

### Community 81 - "check-budgets.mjs"
Cohesion: 0.10
Nodes (16): assets, assetsDir, budgets, cssAssets, cssGzipBytes, distDir, indexHtml, indexHtmlPath (+8 more)

### Community 82 - "logStore.ts"
Cohesion: 0.15
Nodes (17): SessionSummary, ChatThread, buildSearchIndex(), BuildSearchIndexInput, compact(), filterSearchIndex(), getTaskSearchTitle(), normalize() (+9 more)

### Community 83 - "memory_engine.rs"
Cohesion: 0.26
Nodes (19): compact_low_confidence(), create_memory_snapshot(), FrozenMemorySnapshot, generate_memory_hints(), is_duplicate_memory(), MemoryCompactRequest, MemoryCompactResponse, MemoryHint (+11 more)

### Community 84 - "dependencies"
Cohesion: 0.11
Nodes (19): dependencies, class-variance-authority, clsx, i18next, lucide-react, react, react-dom, react-i18next (+11 more)

### Community 85 - "SkillPanel.tsx"
Cohesion: 0.27
Nodes (10): append_audit_event(), integrity_report(), Option, Path, PathBuf, Result, String, Value (+2 more)

### Community 86 - "useTerminalStore"
Cohesion: 0.27
Nodes (7): AuditService, credential_marker_detects_complete_signed_history_deletion(), reconcile_history_marker(), Self, serialized_integrity_report_exposes_only_fixed_path_free_fields(), signed_history_fails_closed_when_its_marker_cannot_be_persisted(), S

### Community 87 - "chatProvider.ts"
Cohesion: 0.11
Nodes (43): argumentValue(), assertRegularAsset(), assertTauriVersionCompatibility(), assertVersionConsistency(), cargoPurl(), cargoRustVersion(), cargoVersion(), collectInventory() (+35 more)

### Community 89 - "asString"
Cohesion: 0.19
Nodes (19): asString(), comparePaths(), createYamlParser(), duplicateRouteRegistryValues(), extractCatalogRecords(), extractStringProperty(), listYamlFiles(), loadCatalogSchemas() (+11 more)

### Community 90 - "next_run_from_expression"
Cohesion: 0.24
Nodes (17): next_daily(), next_run_from_expression(), next_weekday(), normalize(), parse_interval_duration(), parse_time(), parse_weekday(), parses_daily_expression() (+9 more)

### Community 91 - "LlmProfilesPanel.tsx"
Cohesion: 0.09
Nodes (23): ConnectorPanel(), CATEGORIES, CategoryKey, EMPTY_GATEWAY_HEALTH, EMPTY_STARTUP_RECOVERY, GatewayDiagnosticsPanel(), GatewayHealth, GatewaySubsystem (+15 more)

### Community 92 - "compilerOptions"
Cohesion: 0.11
Nodes (17): compilerOptions, allowImportingTsExtensions, erasableSyntaxOnly, lib, module, moduleDetection, moduleResolution, noEmit (+9 more)

### Community 93 - "Implementation Status: ✅ COMPLETE"
Cohesion: 0.11
Nodes (17): 1. Type System, 2. Store Layer, 3. Engine Layer, 4. Database Layer, 5. UI Layer, 6. Tests, Conclusion, Files Modified (10 files) (+9 more)

### Community 94 - "Changes Made"
Cohesion: 0.11
Nodes (17): Backward Compatibility, Behavior, Changes Made, Core Types, Database, Engine Core, Files Modified, How to Use (+9 more)

### Community 95 - "scripts"
Cohesion: 0.07
Nodes (27): scripts, budgets:build, build, dev, dev:tauri, doctor, doctor:ci, i18n:audit (+19 more)

### Community 96 - "skill_engine.rs"
Cohesion: 0.32
Nodes (13): analyze_for_improvement(), analyze_for_skill_generation(), derive_skill_name(), match_skill_for_input(), Arc, Option, String, simple_pattern_match() (+5 more)

### Community 97 - "process_manager.rs"
Cohesion: 0.10
Nodes (34): AttachmentPromptBuildResult, buildAttachmentPromptContext(), collectSnippets(), extractQueryTerms(), ExtractTextLimitedResponse, FsAttachmentMetadataEntry, FsAttachmentMetadataResponse, ImportedAttachmentResponse (+26 more)

### Community 98 - "Fix for Per-Chat/Per-Task Permission Mode and Allowed Directories"
Cohesion: 0.12
Nodes (16): 1. Type Definitions, 2. Store Updates, 3. Engine Core Updates, 4. Database Schema, 5. UI Components, 6. CSS, Benefits, Fallback Behavior (+8 more)

### Community 99 - "5.3 Plan-, Approval- und Task-UI"
Cohesion: 0.10
Nodes (34): add_column_if_missing(), audit_event_insert(), configure_connection(), connection_pragmas_enforce_integrity_and_contention_policy(), corrupt_database_files_and_invalid_data_directories_are_rejected(), create_pre_migration_backup(), current_schema_version(), database_error() (+26 more)

### Community 100 - "Feature Implementierungsvergleich"
Cohesion: 0.12
Nodes (15): 1) CMD/Bash Ausfuehrung, 2) Screenshots / Computer-Use, 3) Word/PowerPoint Integration, Bewertungsbasis, Deep Dive: CMD, Screenshot, Word/PPT, Empfohlene naechste Entscheidung, Feature Implementierungsvergleich, Konkrete Uebernahme-Kandidaten (Priorisiert) (+7 more)

### Community 101 - "Open Cowork"
Cohesion: 0.12
Nodes (16): Build A Windows Installer, Contributing, Current Scope, Documentation, Highlights, License And Disclaimer, MCP Example, Ollama Setup (+8 more)

### Community 102 - "insights.rs"
Cohesion: 0.25
Nodes (14): build_summary(), CategoryCount, EventSummary, InsightsEventRequest, InsightsQueryRequest, InsightsSummary, record_event(), Arc (+6 more)

### Community 103 - "Current Architecture"
Cohesion: 0.25
Nodes (5): Current Canonical Docs, Documentation Source Of Truth, Frontmatter Contract, Precedence, Update Rule

### Community 104 - "FEHLENDE_FEATURES_IM_CODE.md"
Cohesion: 0.13
Nodes (14): 10) Security und Compliance (Kapitel 7.10, IDs 216-230), 11) Enterprise Controls, Monitoring, Betrieb (Kapitel 7.11, IDs 231-240), 12) UX Foundation und Produktivitaetsfeatures (Kapitel 7.1 UX, IDs 241-250), 13) Performance und Stabilitaet (Kapitel 7.1 Performance, IDs 251-254), 14) QA- und Security-Testtiefe (Anforderungsnah, ueber aktuellen Stand hinaus), 1) Plattform und Windows-Basis (Kapitel 7.1, IDs 1-25), 3) Task-Orchestrierung (fortgeschritten) und Personalisierung (Kapitel 7.3, IDs 41-70, 171-185), 4) Dateisystem und File Safety Layer (Kapitel 7.4, IDs 71-100) (+6 more)

### Community 105 - "duckduckgo-websearch-server.mjs"
Cohesion: 0.20
Nodes (12): url, decodeHtmlEntities(), DEFAULTS, extractTargetUrl(), formatTextResult(), normalizeSafeSearch(), parseResultsFromHtml(), rl (+4 more)

### Community 107 - "gateway_health"
Cohesion: 0.04
Nodes (77): authorize_worker_sandbox_source(), claude_code_list_commands(), claude_code_list_tools(), claude_code_send(), claude_code_send_stream(), claude_code_start(), claude_code_status(), claude_code_stop() (+69 more)

### Community 108 - "worker_sandbox.rs"
Cohesion: 0.26
Nodes (17): copy_dir_recursive(), CopyStats, destroy_workspace_snapshot(), prepare_workspace_snapshot(), HashSet, Path, PathBuf, Result (+9 more)

### Community 109 - "modelCapabilities.ts"
Cohesion: 0.18
Nodes (5): AgentCoordinator, AgentInstance, appendAgentRunEvent(), DEFAULT_AGENTS, stringifyRunPayload()

### Community 110 - "Development und Betrieb"
Cohesion: 0.14
Nodes (14): Backup und Recovery (Initialfassung), Build, CI-Gates (aktueller Stand), Development und Betrieb, Empfohlene Betriebsgrenzen (vorlaeufig), Fall A: `npm run tauri build` erzeugt keine EXE, Fall B: Ollama nicht erreichbar, Fall C: Build in CI rot (+6 more)

### Community 112 - "README.md"
Cohesion: 0.29
Nodes (7): 1. Lokale Desktop-Tools fuer Gemma/Ollama, 2. Screenshot-MCP, 3. `ComputerUseAppTest`, Bekannte Namensfalle, Desktop-Steuerung und Computer Use, Uebersicht, Wann welchen Pfad nutzen?

### Community 113 - "Decisions"
Cohesion: 0.15
Nodes (12): CD-001: Local-first is the default product model, CD-002: Projects organize context, not isolated workspaces, CD-003: Work tasks are separate from chat threads, CD-004: Scheduling persists executable snapshots, CD-005: Crew agents synchronize with personality profiles, CD-006: Tool execution is policy-governed, CD-007: Persistence is intentionally mixed today, CD-008: File, desktop, Office, PDF, shell, and MCP capabilities cross the Tauri boundary (+4 more)

### Community 114 - "workTasksStore.ts"
Cohesion: 0.09
Nodes (37): hydrateMcpServer(), initializeCredentialVault(), initializeCredentialVaultOnce(), migrateOrRead(), resetCredentialInitializationForTests(), SENTINELS, connectorLocator(), CredentialLocator (+29 more)

### Community 115 - "build.rs"
Cohesion: 0.36
Nodes (11): cargo_home(), is_dir(), main(), prepare_webview2_loader(), push_build_candidates(), push_registry_candidates(), Option, Path (+3 more)

### Community 116 - "Architektur"
Cohesion: 0.17
Nodes (12): 1. Frontend (`app/src`), 2. Tauri Shell (`app/src-tauri/src/lib.rs`), 3. Ollama Client (`app/src-tauri/src/ollama.rs`), 4. MCP Client (`app/src-tauri/src/mcp.rs`), 5. SQLite Persistenz (`app/src-tauri/src/db.rs`), Architektur, Komponenten, Routing & Layout (+4 more)

### Community 117 - "Decisions"
Cohesion: 0.17
Nodes (11): Current Foundation, Decisions, Design System Decisions, DS-001: Use existing CSS tokens first, DS-002: Desktop productivity shell is the base layout, DS-003: Icons come from `lucide-react`, DS-004: Cards and panels have different roles, DS-005: Theme and density are user preferences (+3 more)

### Community 118 - "Ollama Konfiguration"
Cohesion: 0.18
Nodes (10): 1. Endpoint nicht erreichbar, 2. Modell nicht vorhanden, 3. Timeouts, API-Endpunkte, Beispiel: lokaler Start von Ollama, Fehlerbilder und Diagnose, Konfigurationsquellen, Ollama Konfiguration (+2 more)

### Community 119 - "generateIndex"
Cohesion: 0.18
Nodes (11): addGroup(), buildDuplicateRows(), buildMissingHints(), catalogKindTitle(), countDocs(), formatDocElements(), generateCatalogMarkdown(), generateIndex() (+3 more)

### Community 120 - "Desktop Smoke Test"
Cohesion: 0.20
Nodes (10): Desktop Smoke Test, Desktop-Steuerung validieren, Durchklickrunde, Fehlerprotokoll, Minimalfreigabe, OpenAI Computer Use Debugging, Smoke-Start, Voraussetzungen (+2 more)

### Community 121 - "Decisions"
Cohesion: 0.20
Nodes (9): DD-001: Current docs live outside V1 planning artifacts, DD-002: Seed docs describe implemented behavior only, DD-003: Frontmatter is required for current source-of-truth docs, DD-004: The old architecture doc is superseded, not deleted, DD-005: Keep canonical docs concise, DD-006: Code changes should update docs at the ownership boundary, Decisions, Documentation Decisions (+1 more)

### Community 122 - "buildProductDoc"
Cohesion: 0.31
Nodes (10): buildProductDoc(), coversRouteInventory(), ELEMENT_COLLECTION_FIELDS, firstPresent(), firstStringFromFields(), normalizeElementList(), normalizeRoutes(), normalizeStringList() (+2 more)

### Community 123 - "parseBlock"
Cohesion: 0.31
Nodes (10): isIgnorableLine(), isKeyValue(), parseBlock(), parseFrontmatter(), parseKeyValue(), parseListBlock(), parseScalar(), parseYamlScalar() (+2 more)

### Community 124 - "package.json"
Cohesion: 0.22
Nodes (8): description, license, name, private, repository, type, type, version

### Community 125 - "Open Cowork App"
Cohesion: 0.22
Nodes (8): Checks, Important Paths, Installer, License, Local Development, Notes, Open Cowork App, Stack

### Community 126 - "doctor.mjs"
Cohesion: 0.21
Nodes (10): addCheck(), checks, ciMode, commandVersion(), missingOptional, missingRequired, npmInvocation, root (+2 more)

### Community 127 - "WorkTaskRow"
Cohesion: 0.28
Nodes (3): delete_work_task_removes_matching_schedule(), work_task_lifecycle_round_trip(), WorkTaskRow

### Community 128 - "SettingsView.tsx"
Cohesion: 0.10
Nodes (26): ClaudeMcpServer, exampleJson(), McpCallResponse, McpProbeResponse, McpRuntimeServerStatus, McpTool, McpView(), parseEnv() (+18 more)

### Community 129 - "Runtime Compatibility"
Cohesion: 0.22
Nodes (8): Browser Versus Tauri, Compatibility Update Rule, Current Target, Development Commands, Development Runtime, Packaging Runtime, Provider Runtime Modes, Runtime Compatibility

### Community 130 - "5.2 Cowork-Hauptansicht"
Cohesion: 0.18
Nodes (14): ArtifactVersionResponse, createDocument(), DocumentPreviewPage, DocumentPreviewResponse, DocumentWorkspaceItem, DocumentWorkspaceState, inferFormat(), isDocumentWorkspacePath() (+6 more)

### Community 131 - "Contributing"
Cohesion: 0.25
Nodes (7): Before You Start, Contributing, Contribution License, Development Setup, Pull Request Checklist, Security And Privacy, Validation

### Community 132 - "Product Documentation Index"
Cohesion: 0.25
Nodes (8): Duplicate Statuses, Element Counts, Missing-Doc Hints, Product Documentation Index, Route Map, Schema Errors, Source Documents, Summary

### Community 135 - "Decisions"
Cohesion: 0.29
Nodes (6): Decisions, Legacy Surface Decisions, LS-001: Remove unrouted duplicate UI surfaces, LS-002: Keep PlanApproval compatibility data, not the old task screen, LS-003: Keep RightSidebar panels, remove the unused container export, Review Rule

### Community 136 - "6. Querschnittliche UI-Anforderungen"
Cohesion: 0.29
Nodes (7): 6. Querschnittliche UI-Anforderungen, UI-070 Globale Suche, UI-071 Command Palette, UI-072 Shortcuts, UI-073 Benachrichtigungen, UI-074 Lade-, Leer- und Fehlerzustaende, UI-075 Statuskonsistenz

### Community 138 - "default.json"
Cohesion: 0.33
Nodes (5): description, identifier, permissions, $schema, windows

### Community 140 - "Claude-Code Feature Adoption (Open_Cowork)"
Cohesion: 0.33
Nodes (5): 1) Extrahierte Claude-Code Feature-Familien, 2) Jetzt in Open_Cowork integriert (dieser Slice), 3) Bereits vorher vorhanden und Claude-nah, 4) Noch offen fuer "vollstaendige" Claude-Paritaet, Claude-Code Feature Adoption (Open_Cowork)

### Community 141 - "3. Gestaltungsprinzipien"
Cohesion: 0.33
Nodes (6): 3. Gestaltungsprinzipien, UI-001 Klarer Arbeitskontext, UI-002 Progressive Offenlegung, UI-003 Kontrollierbarkeit, UI-004 Desktop-First Bedienung, UI-005 Konsistenz

### Community 142 - "uniqueStrings"
Cohesion: 0.40
Nodes (6): buildRouteRows(), compareRoutes(), formatStatuses(), mergeRoutes(), readAppRoutes(), uniqueStrings()

### Community 144 - "Current Routes"
Cohesion: 0.40
Nodes (4): Current Routes, Route Update Rule, Settings Sections, Shell Decisions

### Community 145 - "4. Informationsarchitektur"
Cohesion: 0.40
Nodes (5): 4. Informationsarchitektur, UI-010 Hauptnavigation, UI-011 Linke Sidebar, UI-012 Hauptarbeitsflaeche, UI-013 Rechte Kontextzone

### Community 146 - "Copilot Project Instructions"
Cohesion: 0.40
Nodes (4): Completion Checklist, Copilot Project Instructions, Goal, Required Behavior

### Community 147 - "Memory And Skill Discipline"
Cohesion: 0.40
Nodes (4): Definition Of Done, Memory And Skill Discipline, Purpose, Steps

### Community 148 - "Skills Index"
Cohesion: 0.40
Nodes (4): Available Skills, Maintenance Rule, memory-and-skill-discipline, Skills Index

### Community 149 - "openaiCompatibleClient.test.ts"
Cohesion: 0.50
Nodes (3): hasTauriRuntimeMock, readToolDef, safeInvokeMock

### Community 151 - "skill-and-memory.instructions.md"
Cohesion: 0.50
Nodes (3): Quality Rules, Trigger Cases, Workflow

### Community 152 - "Agent Memory Test Tasks"
Cohesion: 0.50
Nodes (3): Agent Memory Test Tasks, Prompts, Verification rule

### Community 155 - "McpView.tsx"
Cohesion: 0.36
Nodes (6): Assert-CodeSigningCertificate(), Assert-InstallerSignature(), ConvertTo-ProcessArgument(), Invoke-BoundedProcess(), Normalize-Thumbprint(), Test-CodeSigningEku()

### Community 160 - "validateProductDoc"
Cohesion: 0.67
Nodes (3): ELEMENT_FIELDS, frontmatterHasList(), validateProductDoc()

### Community 177 - "createDesktopscreenshotAttachment"
Cohesion: 0.33
Nodes (7): captureDesktopVerificationAttachment(), createDesktopscreenshotAttachment(), createMcpscreenshotAttachment(), createToolStreamId(), formatDesktopscreenshotSummary(), parseBase64DataUrl(), parseMcpscreenshotPayload()

### Community 178 - "ui-quality.spec.ts"
Cohesion: 0.33
Nodes (3): PRODUCT_SURFACES, ProductSurface, VIEWPORTS

### Community 180 - "sensitive_data.rs"
Cohesion: 0.14
Nodes (22): bounded_json_stays_valid_and_redacted(), bounds_utf8_text_without_splitting_characters(), diagnostic_label(), is_sensitive_key(), normalized_key(), recursively_redacts_sensitive_keys_and_nested_environment_maps(), redact_and_bound_json_text(), redact_and_bound_optional_json() (+14 more)

### Community 182 - "ChatTurnRequest"
Cohesion: 0.09
Nodes (31): audit_event(), capture_screenshot_for_display_payload(), chat_turn_stream_cancel(), ChatStreamRegistry, detect_crew_cycle(), fs_watch_start(), fs_watch_stop(), local_screenshot_mcp_call() (+23 more)

### Community 183 - "Audit Integrity and Threat Model Contract"
Cohesion: 0.29
Nodes (6): Audit Integrity and Threat Model Contract, Guarantees and non-goals, Protected assets and trust boundaries, Rotation and legacy compatibility, Signed record format, Status and product behavior

### Community 185 - "resolveShellNavigationTarget"
Cohesion: 0.67
Nodes (3): inferShellCwdFromCommand(), normalizeShellPath(), resolveShellNavigationTarget()

### Community 189 - "Release Supply Chain Contract"
Cohesion: 0.29
Nodes (6): Failure and change policy, Locked source and toolchain, Release evidence, Release Supply Chain Contract, Vulnerability and license policy, Windows Authenticode

### Community 212 - "workTaskCrewRuntime.ts"
Cohesion: 0.15
Nodes (25): appendCrewLiveEntry(), assignCrewAgentColor(), buildCrewLiveTitle(), buildCurrentRunContext(), buildFreshNewsGuidelines(), buildWorkTaskCrewGuidelines(), classifyCrewLog(), cleanCrewLogDetail() (+17 more)

### Community 213 - "workTasksStore.ts"
Cohesion: 0.12
Nodes (22): BackendWorkTask, mapBackendWorkTask(), mergeTaskPatch(), migrateLegacyStorageToSqlite(), normalizeRunner(), normalizeStatus(), normalizeTask(), optionalString() (+14 more)

### Community 214 - "pipelineStore.ts"
Cohesion: 0.13
Nodes (17): ActiveTab, PipelinePanel(), BackendPipelineExecutionResult, getLocalPipelines(), hasString(), isRecord(), normalizeRpcPipeline(), ParsedPipelineStep (+9 more)

### Community 215 - "crewHandler.ts"
Cohesion: 0.15
Nodes (21): EngineEvent, buildCrewRunOutput(), createCrewStreamId(), CrewTaskMessageParams, handleCrewTaskMessage(), applyCrewDefaultModel(), augmentCrewToolsForTask(), buildCrewLiveMessageContent() (+13 more)

### Community 216 - "chatStore.ts"
Cohesion: 0.10
Nodes (15): AskQuestionOption, ChatMessage, ChatState, CrewLiveSeverity, CrewLiveStatus, DbMessage, getActiveThread(), isTauriRuntime() (+7 more)

### Community 217 - "chatProvider.ts"
Cohesion: 0.13
Nodes (18): parseThreadProviderSettings(), serializeThreadProviderSettings(), DefaultLlmProfileIds, OllamaConfig, CHAT_PROVIDER_LABELS, CHAT_PROVIDER_OPTIONS, ChatProviderContext, ChatProviderKind (+10 more)

### Community 218 - "QueryEngine"
Cohesion: 0.19
Nodes (6): QueryEngine, ContentBlock, ApprovalRequest, ApprovalResult, AppState, ToolUIRequest

### Community 219 - "ProjectView.tsx"
Cohesion: 0.16
Nodes (15): formatDate(), getProjectTitleForThread(), ProjectView(), readDraggedThreadId(), navigateMock, useChatStore, useProjectStore, AppMode (+7 more)

### Community 220 - "taskStore.ts"
Cohesion: 0.13
Nodes (17): isTauriRuntime(), normalizeRiskLevel(), normalizeStepState(), normalizeTaskStatus(), PermissionConfig, persistInvoke(), PlanApproval, PlanApprovalStatus (+9 more)

### Community 221 - "SkillPanel.tsx"
Cohesion: 0.15
Nodes (13): emptyForm, formatDateTime(), getRunModeLabel(), getTabLabel(), randomId(), runModeLabels, SkillPanel(), SkillTab (+5 more)

### Community 222 - "messageDisplay.ts"
Cohesion: 0.26
Nodes (13): AssistantPresentationOptions, buildModelDebugContent(), escapeRegExp(), extractThinkingContent(), extractThinkingFallback(), resolveAssistantPresentation(), resolveDisplayedAssistantContent(), resolveDisplayedThinkingContent() (+5 more)

### Community 223 - "webSearchSources.ts"
Cohesion: 0.35
Nodes (9): HighlightedChatText(), HighlightedChatTextProps, appendWebSearchSources(), extractWebSearchSources(), formatWebSearchSourcesBlock(), mergeWebSearchSources(), normalizeUrl(), parseWebSearchSourcesFromToolResult() (+1 more)

### Community 224 - "claudeBridge.ts"
Cohesion: 0.20
Nodes (7): ClaudePermissionMode, buildClaudeSystemAddendum(), isToolDeniedByRules(), ParsedSlashCommand, SLASH_COMMAND_DEFINITIONS, SlashCommandDefinition, wildcardToRegex()

### Community 225 - "InsightsPanel.tsx"
Cohesion: 0.36
Nodes (6): formatDateTime(), getLocale(), InsightsPanel(), MetricTone, invokeMock, useInsightsStore

### Community 226 - "followUpPrompt.ts"
Cohesion: 0.42
Nodes (7): buildClarificationContinuationPrompt(), ClarificationContext, CLARIFYING_QUESTION_PATTERNS, FollowUpPromptMessage, inferClarificationContext(), isLikelyClarifyingQuestion(), isLikelyShortFollowUpAnswer()

### Community 227 - "processStore.ts"
Cohesion: 0.36
Nodes (6): ProcessPanel(), ManagedProcess, ProcessStartResult, ProcessState, ProcessStatusResult, useProcessStore

### Community 229 - "GuidedOnboarding.tsx"
Cohesion: 0.38
Nodes (5): GuidedOnboarding(), GuidedOnboardingProps, persistState(), readStoredState(), baseProps

### Community 230 - "taskTemplatesStore.ts"
Cohesion: 0.29
Nodes (4): TaskTemplate, TaskTemplateInput, TaskTemplatesState, useTaskTemplatesStore

### Community 231 - "Current Architecture"
Cohesion: 0.29
Nodes (7): Agent Runtime Boundary, Backend Boundary, Current Architecture, Frontend Boundary, Persistence Boundary, System Shape, Transitional Notes

### Community 232 - "CrewLiveMonitor.test.tsx"
Cohesion: 0.33
Nodes (3): AGENT_COLORS, CrewLiveEntry, CrewLiveState

### Community 234 - "schedulerUtils.ts"
Cohesion: 0.50
Nodes (3): ParsedScheduleInput, parseScheduledTaskInput(), WEEKDAY_PREFIXES

### Community 235 - "5.2 Cowork-Hauptansicht"
Cohesion: 0.40
Nodes (5): 5.2 Cowork-Hauptansicht, UI-030 Prompteingabe, UI-031 Nachrichtenverlauf, UI-032 Thinking-, Status- und Streaming-Darstellung, UI-033 Folgeaktionen

### Community 236 - "5.3 Plan-, Approval- und Task-UI"
Cohesion: 0.40
Nodes (5): 5.3 Plan-, Approval- und Task-UI, UI-040 Planansicht, UI-041 Approval-Interaktion, UI-042 Task-Lifecycle, UI-043 Schritt- und Artefakttransparenz

### Community 237 - "8. Accessibility"
Cohesion: 0.40
Nodes (5): 8. Accessibility, UI-090 Tastaturbedienung, UI-091 Screenreader- und Semantik-Anforderungen, UI-092 Kontrast und Lesbarkeit, UI-093 Zoom und Skalierung

### Community 238 - "7. Desktop- und Fensterverhalten"
Cohesion: 0.50
Nodes (4): 7. Desktop- und Fensterverhalten, UI-080 Responsives Desktop-Layout, UI-081 Fensterzustand und Persistenz, UI-082 Fokusmanagement

### Community 239 - "9. Visuelle und textliche Anforderungen"
Cohesion: 0.50
Nodes (4): 9. Visuelle und textliche Anforderungen, UI-100 Design System, UI-101 Dichte und Ruhe, UI-102 Sprache und Mikrotexte

## Knowledge Gaps
- **1174 isolated node(s):** `CoworkView`, `SettingsView`, `TasksView`, `CrewView`, `ProjectView` (+1169 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **50 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Command` connect `index.ts` to `crew_python_bridge.rs`, `claude_code_bridge.rs`, `registry.ts`?**
  _High betweenness centrality (0.285) - this node is a cross-community bridge._
- **Why does `suppress_command_window()` connect `crew_python_bridge.rs` to `index.ts`?**
  _High betweenness centrality (0.283) - this node is a cross-community bridge._
- **Why does `Database` connect `useConfigStore` to `Result`, `String`, `work_task_upsert`, `AppHandle`, `tr`, `workTaskCrewRuntime.ts`, `String`, `LeftSidebar.tsx`, `sensitive_data.rs`, `db.rs`, `ChatTurnRequest`, `crew_provider_health_check`, `.open_in_memory`, `RequestContext`, `terminal_backends.rs`, `memory_engine.rs`, `taskStore.ts`, `skill_engine.rs`, `5.3 Plan-, Approval- und Task-UI`, `insights.rs`, `gateway_health`, `WorkTaskRow`?**
  _High betweenness centrality (0.161) - this node is a cross-community bridge._
- **What connects `CoworkView`, `SettingsView`, `TasksView` to the rest of the system?**
  _1175 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Result` be split into smaller, more focused modules?**
  _Cohesion score 0.04710793082886106 - nodes in this community are weakly interconnected._
- **Should `crewStore.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.035003431708991076 - nodes in this community are weakly interconnected._
- **Should `registry.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.028985507246376812 - nodes in this community are weakly interconnected._